...
PanelWindow {
  ...
  // LOCAL CHANGE (not upstream Omarchy): screen-edge clearance is kept
  // separate from the bar gap. `gap` below is 0 in this shell so popups sit
  // flush under the bar, and sharing one value meant the clamp in cardOrigin
  // went to 0 too, so right-hand popups ended up touching the screen edge.
  property int margin: bar && bar.popupEdgeMargin !== undefined ? bar.popupEdgeMargin : Style.gapsOut
  ...
  // LOCAL CHANGE (not upstream Omarchy): inverted corner fillets.
  //
  // A flush card with squared top corners still meets the bar as a butt joint.
  // These two concave wedges sit just outside the card on the bar's edge and
  // are filled with the card colour, so the bar appears to flow into the popup
  // — the same shape components/popout draws beside its own popups.
  //
  // Each wedge is a square of the corner radius with one arc cut out of it:
  // fill the square, then sweep an arc from the outer corner back to the
  // origin, leaving the concave quarter.
  component CornerFillet: Shape {
    id: fillet

    // Which side of the card this one flanks, and which way the bar is.
    property bool mirrored: false
    // Clamped to the card, not fixed at the radius. While the card is still
    // springing open it is shorter than the radius, and a fixed-size fillet
    // hangs below it with its arc meeting nothing — the popup reads as a
    // floating pill flanked by two stray wedges that only join the bar once
    // the card has grown past it. Shrinking with the card keeps the joint
    // whole for every frame of the animation.
    readonly property real size: Math.max(0, Math.min(card.radius,
      root.barPos === "top" || root.barPos === "bottom" ? card.height : card.width))

    width: size
    height: size
    visible: card.fusedToBar && (root.barPos === "top" || root.barPos === "bottom") && opacity > 0.01
    opacity: card.opacity
    // Without a layer the arc edge aliases badly against the card colour.
    layer.enabled: true
    layer.samples: 4

    x: mirrored ? card.x + card.width : card.x - size
    y: root.barPos === "bottom" ? card.y + card.height - size : card.y

    transform: [
      Scale {
        origin.x: fillet.size / 2
        origin.y: fillet.size / 2
        yScale: root.barPos === "bottom" ? -1 : 1
      }
    ]
    ...
    ShapePath {
      ...
      startX: fillet.mirrored ? fillet.size : 0
      startY: 0

      PathLine {
        x: fillet.mirrored ? 0 : fillet.size
        y: 0
      }
      ...
      PathArc {
        x: fillet.mirrored ? fillet.size : 0
        y: 0
        radiusX: fillet.size
        radiusY: fillet.size
        direction: fillet.mirrored ? PathArc.Clockwise : PathArc.Counterclockwise
      }
    }
  }
  ...
  BorderSurface {
    id: card
    // LOCAL CHANGE (not upstream Omarchy): grow-from-the-bar open animation,
    // matching the popups in components/popout. Those start as a small pill
    // under the bar and spring out to full size with the content clipped, so
    // the popup reads as unfolding out of the bar rather than fading in over
    // it. Upstream only cross-fades opacity.
    //
    // The collapsed size and the curves come from `bar`, so they stay tunable
    // from one place alongside the rest of the panel styling.
    readonly property bool shown: root.open || root.popoutSwitching

    // Open/close progress, 0 collapsed to 1 full. The animation lives here
    // rather than on width/height, because a panel's contentHeight keeps
    // moving after the popup is up — a network scan fills in, a section
    ...
    // LOCAL CHANGE (not upstream Omarchy): when the card sits flush against
    // the bar (gap 0), the two corners on the bar's edge are squared off so
    // the popup reads as an extension of the bar rather than a separate
    // floating card — matching the popups in components/popout, which get the
    // same effect by clipping a rounded rect that overhangs upward.
    // Only meaningful while flush; with a gap the rounded corners are right.
    readonly property bool fusedToBar: root.gap === 0
      && (!root.bar || root.bar.popupFuseWithBar !== false)
    topLeftRadius: fusedToBar && (root.barPos === "top" || root.barPos === "left") ? 0 : radius
    ...
    Item {
      ...
      // LOCAL CHANGE (not upstream Omarchy): laid out at the card's FINAL
      // size, not at its animated size.
      //
      // Every panel sets contentHeight from its own content
      // (fittedContentHeight over a column's implicitHeight), and that column
      // lives in here. With anchors.fill the holder shrank with the animating
      // card, the column re-wrapped, implicitHeight changed, and contentHeight
      // — the very target the card was animating toward — moved with it. The
      // bottom edge chased a target that was reacting to its own position,
      // which is the wobble that survived every easing-curve change.
      //
      // Pinning the holder to the final size breaks that loop: the content is
      // measured once, and card.clip reveals it as the card grows. This is how
      // the popups in components/popout are built — a fixed-size content Item
